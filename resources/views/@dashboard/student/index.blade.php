    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
                ...
                <h5 class="content-header-title float-left pr-1 mb-0">{{ trans('student.students') }}</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-light table-striped nowrap zero-configuration">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('student.display_name') }}</th>
                                    <th>{{ trans('student.email') }}</th>
                                    <th>{{ trans('student.phone') }}</th>
                                    <th>{{ trans('student.info') }}</th>
                                    <th>{{ trans('student.social_facebook_url') }}</th>
                                    <th>{{ trans('student.social_twitter_url') }}</th>
                                    <th>{{ trans('student.social_linkedin_url') }}</th>
                                    <th>{{ trans('student.social_youtube_url') }}</th>
                                    <th>{{ trans('student.is_active') }}</th>
                                ...
                                <tbody>
                                    @foreach($resources as $key => $resource)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @if(check_authority('edit.student'))
                                                        <a href="{{ route('student.edit', [$resource->uuid]) }}" class="btn btn-sm btn-success">
                                                            <i class="bx bx-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if(check_authority('delete.student'))
                                                        <a href="{{ route('student.destroy', [$resource->uuid]) }}" class="confirm-delete btn btn-sm btn-danger">
                                            ...
                                            <td>{{ $resource->info }}</td>
                                            <td>{{ $resource->social_facebook_url }}</td>
                                            <td>{{ $resource->social_twitter_url }}</td>
                                            <td>{{ $resource->social_linkedin_url }}</td>
                                            <td>{{ $resource->social_youtube_url }}</td>
                                            <td>
