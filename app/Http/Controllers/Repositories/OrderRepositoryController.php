...
class OrderRepositoryController extends Controller
{
    ...
    public function index($request)
    {
        ...
        // Get All Resource
        $data_request = $this->get_data(new Order(), $request, [
            "name" => "like",
        ]);

        $data = $this->general_response($this->success_list_message(), OrderResource::collection($data_request['resources']->get()), $data_request['resources']->count(), false);
    ...
    public function store($request)
    {
        ...
        // Return
        if($resource){
            $data = $this->general_response($this->success_create_message(), new OrderResource($resource), 1);
        }else{
    ...
    {
        // Check Permissions
        if(!check_authority('edit.order')){
            return $this->general_response($this->fail_permission_message());
        }

        $resource = Order::getOneBy(['uuid' => $uuid]);

        if($resource){
            $data = $this->general_response($this->success_update_message(), new OrderResource($resource), 1, false);
        }else{
    ...
    public function update($request, $uuid)
    {
        ...
        $resource = Order::getOneBy(['uuid' => $uuid]);
        if(!$resource){
            return $this->general_response($this->fail_resource_not_found_message(trans('messages.order')));
        }
        ...
        // Do Code
        try {
            $updatedResource = Order::edit($fields, $resource->id);
        }catch (\Exception $exception){
            $exceptions = 'Message : ' . $exception->getMessage() . ' File : ' . $exception->getFile();
            return $this->general_response($this->fail_exception_message($exceptions));
        }
        ...
        // Return
        if($updatedResource){
            $data = $this->general_response($this->success_update_message(), new OrderResource($resource), 1);
        }else{
            $data = $this->general_response($this->fail_update_message());
        }
    ...
    {
        // Check Permissions
        if(!check_authority('delete.order')){
            return $this->general_response($this->fail_permission_message());
        }
        ...
        $resource = Order::getOneBy(['uuid' => $uuid]);
        if($resource){
            $deletedResource = Order::remove($resource->id);
