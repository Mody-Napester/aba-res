    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
                ...
                <h5 class="content-header-title float-left pr-1 mb-0">{{ trans('order.orders') }}</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-light table-striped nowrap zero-configuration">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('order.speciality') }}</th>
                                    <th>{{ trans('order.details') }}</th>
                                    <th>{{ trans('order.phone') }}</th>
                                    <th>{{ trans('order.email') }}</th>
                                    <th>{{ trans('order.avatar') }}</th>
                                    <th>{{ trans('order.is_active') }}</th>
                                    <th>{{ trans('order.created_by') }}</th>
                                    <th>{{ trans('order.updated_by') }}</th>
                                    <th>{{ trans('order.created_at') }}</th>
                                    <th>{{ trans('order.updated_at') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @if(check_authority('edit.order'))
                                                        <a href="{{ route('order.edit', [$resource->uuid]) }}" class="btn btn-sm btn-success">
                                                            <i class="bx bx-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if(check_authority('delete.order'))
                                                        <a href="{{ route('order.destroy', [$resource->uuid]) }}" class="confirm-delete btn btn-sm btn-danger">
