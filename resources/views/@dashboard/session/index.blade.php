...
@section('title') {{ trans('session.page_title') }} @endsection
    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
                ...
                <h5 class="content-header-title float-left pr-1 mb-0">{{ trans('session.sessions') }}</h5>
                <div class="breadcrumb-wrapper d-none d-sm-block">
                    <ol class="breadcrumb p-0 mb-0 pl-1">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}"><i class="bx bx-home-alt"></i></a></li>
                        <li class="breadcrumb-item active">{{ trans('session.sessions') }}</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-light table-striped nowrap zero-configuration">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('session.course') }}</th>
                                    <th>{{ trans('session.is_published') }}</th>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @if(check_authority('edit.session'))
                                                        <a href="{{ route('session.edit', [$resource->uuid]) }}" class="btn btn-sm btn-success">
                                                            <i class="bx bx-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if(check_authority('delete.session'))
                                                        <a href="{{ route('session.destroy', [$resource->uuid]) }}" class="confirm-delete btn btn-sm btn-danger">
