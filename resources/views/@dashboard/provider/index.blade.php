    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
                ...
                <h5 class="content-header-title float-left pr-1 mb-0">{{ trans('provider.providers') }}</h5>
                <div class="breadcrumb-wrapper d-none d-sm-block">
                    <ol class="breadcrumb p-0 mb-0 pl-1">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}"><i class="bx bx-home-alt"></i></a></li>
                        <li class="breadcrumb-item active">{{ trans('provider.providers') }}</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-light table-striped nowrap zero-configuration">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('provider.display_name') }}</th>
                                    <th>{{ trans('provider.details') }}</th>
                                    <th>{{ trans('provider.class') }}</th>
                                    <th>{{ trans('provider.color') }}</th>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                @if(check_authority('edit.provider'))
                                                    <a href="{{ route('provider.edit', [$resource->uuid]) }}" class="btn btn-sm btn-success">
                                                        <i class="bx bx-edit"></i>
                                                    </a>
                                                @endif
                                                @if(check_authority('delete.provider'))
                                                    <a href="{{ route('provider.destroy', [$resource->uuid]) }}" class="confirm-delete btn btn-sm btn-danger">
