...
@section('title') {{ trans('question.page_title') }} @endsection
    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
                ...
                <h5 class="content-header-title float-left pr-1 mb-0">{{ trans('question.questionzes') }}</h5>
                <div class="breadcrumb-wrapper d-none d-sm-block">
                    <ol class="breadcrumb p-0 mb-0 pl-1">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}"><i class="bx bx-home-alt"></i></a></li>
                        <li class="breadcrumb-item active">{{ trans('question.questionzes') }}</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-light table-striped nowrap zero-configuration">
                                ...
                                <tr>
                                    ...
                                    <th>{{ trans('question.details') }}</th>
                                    <th>{{ trans('question.created_by') }}</th>
                                    <th>{{ trans('question.updated_by') }}</th>
                                    <th>{{ trans('question.created_at') }}</th>
                                    <th>{{ trans('question.updated_at') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @if(check_authority('edit.question'))
                                                        <a href="{{ route('question.edit', [$resource->uuid]) }}" class="btn btn-sm btn-success">
                                                            <i class="bx bx-edit"></i>
                                                        </a>
                                                    @endif
                                                    @if(check_authority('delete.question'))
                                                        <a href="{{ route('question.destroy', [$resource->uuid]) }}" class="confirm-delete btn btn-sm btn-danger">
                                                            <i class="bx bx-trash"></i>
                                                        </a>
                                                    @endif
                                                </div>
                                            </td>
                                            <td>{{ ($resource->quiz)? getTrans($resource->quiz->name, lang()) : '-'}}</td>
