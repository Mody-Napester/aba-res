...
class ConsultationRepositoryController extends Controller
{
    ...
    public function edit($request, $uuid)
    {
        ...
        $resource = Consultation::getOneBy(['uuid' => $uuid]);

        if($resource){
            $data = $this->general_response($this->success_update_message(), new ConsultationResource($resource), 1, false);
        }else{
    ...
    public function update($request, $uuid)
    {
        ...
        $resource = Consultation::getOneBy(['uuid' => $uuid]);
        if(!$resource){
            return $this->general_response($this->fail_resource_not_found_message(trans('messages.consultation')));
        }
        ...
        // Do Code
        try {
            $updatedResource = Consultation::edit($fields, $resource->id);
        }catch (\Exception $exception){
            $exceptions = 'Message : ' . $exception->getMessage() . ' File : ' . $exception->getFile();
            return $this->general_response($this->fail_exception_message($exceptions));
        }
        ...
        // Return
        if($updatedResource){
            $data = $this->general_response($this->success_update_message(), new ConsultationResource($resource), 1);
        }else{
            $data = $this->general_response($this->fail_update_message());
        }
    ...
    public function destroy($request, $uuid)
    {
        ...
        $resource = Consultation::getOneBy(['uuid' => $uuid]);
        if($resource){
            $deletedResource = Consultation::remove($resource->id);
